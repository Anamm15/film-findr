...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 pt-28 sticky top-0 bg-background h-screen border-r border-gray-200 p-4">
      ...
      <nav className="space-y-2">
        {menu.map((item) => (
          <Link
            key={item.name}
            to={item.path[0]}
            className={`block px-4 py-2 rounded-lg ${item.path.includes(location.pathname)
